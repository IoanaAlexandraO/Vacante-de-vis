...
global.obiectGlobal = {
  ...
  folderCss: path.join(__dirname, "resurse/css"), // Corrected to point to CSS folder
...
async function compileazaScss(caleScss, caleCss) {
  // Get absolute path for SCSS file (if relative, consider it relative to folderScss)
  const scssAbs = path.isAbsolute(caleScss)
    ? caleScss
    : path.join(obiectGlobal.folderScss, caleScss);

  // Get absolute path for CSS file (if relative, consider it relative to folderCss)
  // If caleCss is not provided, use the same name as scss but with .css extension
  ...
  // Ensure backup folder exists (create the full path structure if needed)
  const backupPath = path.join(obiectGlobal.folderBackup, "resurse/css");
  await fsp.mkdir(backupPath, { recursive: true });

  try {
    // Backup existing CSS file if it exists
    if (fs.existsSync(cssAbs)) {
      ...
      try {
        ...
        console.log(`[SCSS] Backup creat: ${cssAbs} -> ${caleBackup}`);
      } catch (errBackup) {
        console.error(`[Eroare] La crearea backup-ului pentru ${cssAbs}:`, errBackup.message);
      }
    }
...
function compileazaToateScss() {
  console.log(`[SCSS] Compilare inițială pentru toate fișierele din ${obiectGlobal.folderScss}`);
  const fisiere = fs.readdirSync(obiectGlobal.folderScss).filter(f => f.endsWith(".scss"));
  
  console.log(`[SCSS] Găsite ${fisiere.length} fișiere SCSS pentru compilare`);
